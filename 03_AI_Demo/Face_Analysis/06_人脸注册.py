#人脸识别注册最常见的应用场景就是人脸门禁或者打卡机了，之前的例程都是去识别任意人脸。
#想要去识别特定的人脸，就需要先将你想要识别的人脸注册到系统里，然后再去运行识别的程序。

"""

基本流程:
人脸检测： 首先使用人脸检测模型从输入的图像流中定位人脸的区域、
边界框以及五官关键点（比如眼睛、鼻子、嘴巴的位置）。

人脸特征提取： 在获得人脸关键点位置后，对人脸进行仿射校正对齐标准五官位置，
并使用人脸特征提取模型将对齐后的人脸提取为固定长度的特征向量。

人脸特征注册： 将提取到的人脸特征向量存入本地数据库（其实就是我们运行固件用的TF卡）。
通过数据库文件名或其他标识就可以和这个此特征对应的人名（也就是注册时的ID）关联。

人脸识别： 识别阶段会对输入帧中的人脸同数据库中存储的特征进行相似度计算。
若相似度超过一定阈值（在本例中为0.75），则判断为数据库中某已知人员；否则输出“unknown”。
