...
laser_threshold = (0, 100, -128, 127, -128, 127) # 激光阈值

state = 0     #识别状态
pencil_points = []  #记入铅笔矩形坐标5个点
# 设置追踪阈值
...
def laser_calibration():
    ...
    while loop:
        ...
        if blobs:
            img.draw_cross(blobs[0].x(), blobs[0].y(), color=(255, 255, 255))
            #定义按键，通过按键计入激光点的坐标
            if KEY.value() == 1:
        ...
        for n in range(len(pencil_points)):
            ...
            img.draw_string(pencil_points[n][0], pencil_points[n][1], str(n), color=(255, 255, 255))
            # 发送数据
