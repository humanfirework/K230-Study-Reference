...
# 可调参数
rect_binart = [(84, 200)]  # 或更低，初始二值化阈值，可根据实际情况调整
...
# A4纸比例参数 (210mm x 297mm ≈ 1:1.414)
A4_ASPECT_RATIO = 1.414   # A4纸长宽比
A4_RATIO_TOLERANCE = 0.2  # A4比例容差
...
def find_best_rect(img_binary):
    ...
    for blob in blobs:
        ...
        # A4纸比例过滤
        # 检查矩形长宽比是否接近A4纸比例
        if abs(aspect_ratio - A4_ASPECT_RATIO) > A4_RATIO_TOLERANCE and abs(aspect_ratio - (1/A4_ASPECT_RATIO)) > A4_RATIO_TOLERANCE:
            continue
...
try:
    ...
    while True:
        ...
        # 简化形态学操作 - 减少计算量以提高帧率
        # 使用开运算替代单独的腐蚀和膨胀操作
        img_binary.close(1)  # 开运算：先腐蚀后膨胀，去除小噪点
