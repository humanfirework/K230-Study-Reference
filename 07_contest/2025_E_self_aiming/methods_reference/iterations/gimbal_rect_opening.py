...
threshold_value = 0   # 二值化阈值（0=使用 Otsu 自动阈值）/ Threshold for binarization (0 = Otsu)
...
try:
    ...
    sensor.set_pixformat(Sensor.RGB888, chn=CAM_CHN_ID_0)  # 设置像素格式为 RGB888 / Set pixel format to RGB888
    ...
    while True:
        ...
        img_out = image.Image(picture_width, picture_height, image.GRAYSCALE,
                              alloc=image.ALLOC_REF, data=result_np)

        max_rect = find_best_rect(img_out, is_first_frame, last_frame_center)
