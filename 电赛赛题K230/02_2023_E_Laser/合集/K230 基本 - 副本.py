...
def send_data(x,y):
    ...
    for i in range(4):
        ...
        # 发送x坐标数据包
        # 数据包格式: [0x55, 0xaa, 0xff, , 0xfa]
        # 0x55 0xaa: 数据包头
        # x_h, x_l: x坐标的高低位
        # 0xfa: 数据包尾
        # 合并发送x和y坐标数据包
        combined_packet = bytearray([
            0x55, 0xaa, 0x00, int(x_h)&0xFF, int(x_l)&0xFF,
            int(y_h)&0xFF, int(y_l)&0xFF, 0xfa])
        uart2.write(combined_packet)
...
try:
    ...
    while True:
        ...
        if state == 1:
            ...
            blobs = img.find_blobs([laser_threshold, Red_thresholds])
            if blobs:
                pixel = []
                for B in blobs:
            ...
            #定义按键，通过按键计入激光点的坐标
                if KEY.value() == 1:
                    time.sleep_ms(100)
                    if KEY.value() == 1 and len(pencil_points) < 3:
                        blobs = img.find_blobs([laser_threshold, Red_thresholds])
                        if blobs:
                            pixel = []
                            for B in blobs:
        ...
        if state == 2:   # 接收到‘2’时持续发送
            ...
            blobs = img.find_blobs([Red_thresholds, laser_threshold])  # 重新检测色块 （可添加阈值，识别不同颜色）
